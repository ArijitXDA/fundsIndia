...
// Exact columns in b2b_sales_current_month CSV
const REQUIRED_COLS = ['Arn', 'RM Emp ID'];
const NUMERIC_COLS = [
  'MF+SIF+MSCI', 'COB (100%)', 'COB (50%)', 'AIF+PMS+LAS+DYNAMO (TRAIL)',
  'MF Total (COB 100%)', 'MF Total (COB 50%)', 'ALTERNATE', 'ALT Total',
  'Total Net Sales (COB 100%)', 'Total Net Sales (COB 50%)',
];
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!adminRole?.roles?.includes(1)) {
      return NextResponse.json({ error: 'Role 1 (B2B MTD MIS) required' }, { status: 403 });
    }
    ...
    const totalCOB = rows.reduce((s, r) => s + r['COB (100%)'], 0);
    const totalMF = rows.reduce((s, r) => s + r['MF+SIF+MSCI'], 0);
    const totalAlt = rows.reduce((s, r) => s + r['ALTERNATE'], 0);

    const branchSummary = rows.reduce((acc: any, r) => {
    ...
    if (!confirm) {
      return NextResponse.json({
        ...
        stats: {
          ...
          totalCOB100: totalCOB.toFixed(2),
          totalMFSIFMSCI: totalMF.toFixed(2),
          totalAlternate: totalAlt.toFixed(2),
        },
        zoneSummary: branchSummary,
        ...
        message: `Ready to replace all ${totalRows} records in B2B MTD MIS table.`,
        warning: 'This will DELETE all existing records in b2b_sales_current_month and replace with new data.',
      });
    }

    // Confirmed — replace table contents
    const { error: deleteError } = await supabaseAdmin
      .from('b2b_sales_current_month')
      .delete()
      .neq('Arn', '__never__'); // delete all rows
    ...
    for (let i = 0; i < rows.length; i += 100) {
      ...
      const { error } = await supabaseAdmin.from('b2b_sales_current_month').insert(batch);
    ...
    console.error('[UPLOAD B2B MTD]', error);
