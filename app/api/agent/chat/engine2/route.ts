...
export async function POST(request: NextRequest) {
  ...
  const engineInstruction = `You are Thinking Engine 2, an independent AI analyst. You have access to the same tools as Engine 1 and should use them to independently fetch and verify data. Provide your own analysis — explore angles Engine 1 may have missed, cross-check numbers, and offer a different perspective. Do NOT mention your model name. Use the same formatting rules (charts via \`\`\`chart blocks, bullets, bold) as described in the system prompt.${webSearchBlock}`;
